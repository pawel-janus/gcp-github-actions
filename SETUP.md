...
Complete one-time setup for GitHub Actions → GCP authentication via Workload Identity Federation (WIF).

⚠️ **Complete Steps 1-9 below first, then configure GitHub Repository Variables (Step 10)**
...
# Logging Viewer — read Cloud Build logs (optional)
gcloud projects add-iam-policy-binding $PROJECT_ID \
  --account=$ACCOUNT \
  --member="serviceAccount:github-actions-sa@${PROJECT_ID}.iam.gserviceaccount.com" \
...
**Note:** The `gs://{PROJECT_ID}_cloudbuild` bucket is automatically created by Cloud Build on first use. If the bucket doesn't exist yet, these commands will fail - run your first `gcloud builds submit` (see workspace README for manual deploy) to create it, then come back to add permissions.
...
WIF setup is complete. You can now:
1. See workspace README files for local development and manual deployment
2. Push to `main` to trigger automated deployment via GitHub Actions
